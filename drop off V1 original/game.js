...
// Start menu fire video on the very first touch or click
function startMenuFire() {
    const menuFire = document.getElementById('menuFireVideo');
    if (!menuFire) return;

    // Ensure muted is set (some browsers need the property, not just the attribute)
    menuFire.muted = true;
    // Reset and play
    menuFire.currentTime = 0;
    menuFire.play().catch(function() {
        // If still blocked, try again after a tiny delay
        setTimeout(function() { menuFire.play().catch(function(){}); }, 100);
    });
    
    // Remove both listeners after first interaction
    document.removeEventListener('touchstart', startMenuFire);
    document.removeEventListener('click', startMenuFire);
}

// Listen for the first real user interaction
document.addEventListener('touchstart', startMenuFire, { once: true });
document.addEventListener('click', startMenuFire, { once: true });
...
function gameLoop() {
    ...
    if (!isPaused) {
        ...
        if (gameState === "playing") {
            if (speedX !== 0) {
                x += Math.sign(speedX) * dynamicSpeedX;
            }
            updateBackground();
        }
        ...
        ball.style.transform = `translate(${x}px, ${dynamicBallY}px)`;
        ...
        if (gameSurface) {
            gameSurface.style.transform = `translate(${shakeX}px, ${shakeY}px)`;
        }

        const bgL = document.getElementById('bgLayer');
        if (bgL) {
            bgL.style.transform = `translateY(${-worldY}px)`;
        }
...
function renderObstacles() {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        ob.element.style.transform = `translateY(${screenY}px)`;
...
function resetGameEngine(goToMenu) {
    ...
    ball.style.transform = `translate(${x}px, ${window.innerHeight * 0.15}px)`;
    ball.classList.remove('slamming');
    const fireVideo = document.getElementById('fire-video');
    if (fireVideo) {
        fireVideo.pause();
